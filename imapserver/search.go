...
// tag: 请求标记，用于响应。
// dec: 解码器，用于解析输入数据。
// numKind: 数字类型，表示消息的唯一标识符类型（例如，UID或SEQ）。
func (c *Conn) handleSearch(tag string, dec *imapwire.Decoder, numKind NumKind) error {
	...
	if maybeReadSearchKeyAtom(dec, &atom) && strings.EqualFold(atom, "RETURN") {
		if err := readSearchReturnOpts(dec, &options); err != nil {
			return fmt.Errorf("在 search-return-opts 中: %w", err)
		}
		if !dec.ExpectSP() {
			return dec.Err()
		}
	...
	if strings.EqualFold(atom, "CHARSET") {
		...
		default:
			return &imap.Error{
				...
				Code: imap.ResponseCodeBadCharset, // TODO: 返回支持的字符集列表
				Text: "只支持 US-ASCII 和 UTF-8 作为搜索字符集",
			}
		}
		atom = ""
		maybeReadSearchKeyAtom(dec, &atom)
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("在 search-key 中: %w", err)
		}
...
func (c *Conn) writeESearch(tag string, data *imap.SearchData, options *imap.SearchOptions) error {
	...
	// 当没有结果时，我们需要发送没有 ALL 关键字的 ESEARCH 响应
...
func (c *Conn) writeSearch(numSet imap.NumSet) error {
	...
	if !ok {
		return fmt.Errorf("imapserver: 在 SEARCH 响应中枚举消息编号失败")
	}
	return enc.CRLF()
}

// readSearchReturnOpts 读取搜索返回选项。
// dec: 解码器，用于解析输入数据。
// options: 搜索选项。
...
// readSearchKeyWithAtom 使用原子读取搜索键。
// criteria: 搜索条件。
// dec: 解码器，用于解析输入数据。
// key: 搜索键。
